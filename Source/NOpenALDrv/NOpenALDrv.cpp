...
void UNOpenALAudioSubsystem::InternalClassInitializer( UClass* Class )
{
	...
	new(Class, "DeviceName",         RF_Public)UStrProperty   ( CPP_PROPERTY( DeviceName         ), "Audio", CPF_Config );
...
{
	// Diagnostics are switched on by an environment variable, not a config
	// setting: this constructor runs after the config is read, so anything it
	// assigns here wins over the ini and is then written back over it. Same
	// trap as NSDLDrv's LogPadInput.
	LogAudio = ( getenv( "UT99_LOG_AUDIO" ) != NULL );

	// Nothing else is assigned here on purpose. Every remaining member is a
	// CPF_Config property, and StaticConstructObject copies the config-loaded
	// class defaults into the new object *before* running this constructor -
	// so anything set here silently beats the ini and is then written back
	// over it. That is what pinned MusicVolume at 63/255 whatever the Audio
	// options menu was set to, and left MasterVolume=0 in the saved ini.
	// The defaults now live in the ini section AppRun writes, where the game's
	// own levels can be carried across and the player can actually change them.
}
...
UBOOL UNOpenALAudioSubsystem::Init()
{
	...
	Viewport = NULL;
	Device = NULL;
	// TCHAR is char in this build (the engine logs "Character set: ANSI"), so
	// the string passes straight through to OpenAL.
	if( DeviceName.Len() )
		Device = alcOpenDevice( (const ALCchar*)*DeviceName );
	if( !Device )
	...
	const ALint AttrList[] = {
		...
		// A UBoolProperty stores its value as its own BitMask bit, and which
		// bit that is depends on link order - so this member reads back as 1
		// or 2 for the same "True". Fine for a truth test, not as an ALC
		// attribute value, which must be exactly ALC_TRUE or ALC_FALSE.
		ALC_SOFT_HRTF, UseHRTF ? ALC_TRUE : ALC_FALSE,
		0
	};
	...
	if( LogAudio )
		debugf( TEXT("audio: MasterVolume=%i SoundVolume=%i MusicVolume=%i OutputRate=%i UseReverb=%i"),
			(INT)MasterVolume, (INT)SoundVolume, (INT)MusicVolume, OutputRate, (INT)UseReverb );
...
void UNOpenALAudioSubsystem::RegisterMusic( UMusic* Music )
{
	...
	if( LogAudio )
		debugf( TEXT("audio: RegisterMusic %s handle=%i data=%i"),
			Music->GetName(), Music->Handle?1:0, Music->Data.Num() );
	...
	MusicIsLoaded = true;
	if( LogAudio )
		debugf( TEXT("audio: RegisterMusic %s OK (xmp loaded)"), Music->GetName() );
...
void UNOpenALAudioSubsystem::RegisterSound( USound* Sound )
{
	...
	if( LogAudio )
		debugf( TEXT("audio: registered %s ch=%i bits=%i rate=%i bytes=%i loops=%i alErr=%i"),
			Sound->GetName(), (INT)*WaveInfo.pChannels, (INT)*WaveInfo.pBitsPerSample,
			(INT)*WaveInfo.pSamplesPerSec, WaveInfo.SampleDataSize, WaveInfo.SampleLoopsNum,
			(INT)alGetError() );
...
UBOOL UNOpenALAudioSubsystem::PlaySound( AActor* Actor, INT Id, USound* Sound, FVector Location, FLOAT Volume, FLOAT Radius, FLOAT Pitch )
{
	...
	{
		if( LogAudio )
			debugf( TEXT("audio: DROPPED %s (voice=%i handle=%i prio=%f)"),
				Sound ? Sound->GetName() : TEXT("<none>"), Voice ? 1 : 0,
				(Sound && Sound->Handle) ? 1 : 0, Priority );
		return false;
	}
	...
	if( LogAudio )
		debugf( TEXT("audio: play %s vol=%f radius=%f gain=%f looping=%i"),
			Sound->GetName(), Voice->Volume, Voice->Radius,
			Voice->Volume * ( SoundVolume / 255.f ), (INT)Voice->Looping );
...
void UNOpenALAudioSubsystem::PlayMusic()
{
	...
	if( LogAudio )
		debugf( TEXT("audio: PlayMusic section=%i loaded=%i freeBufs=%i"),
			(INT)MusicSection, (INT)MusicIsLoaded, NumFreeMusicBuffers );
...
void UNOpenALAudioSubsystem::Update( FPointRegion Region, FCoords& Listener )
{
	...
	if( Viewport->Actor && Viewport->Actor->Transition != MTRAN_None )
	{
		...
		if( Music == NULL )
		{
			if( LogAudio )
			{
				static UMusic* LastLogged = (UMusic*)-1;
				static BYTE    LastSection = 254;
				if( Viewport->Actor->Song != LastLogged || Viewport->Actor->SongSection != LastSection )
				{
					LastLogged  = Viewport->Actor->Song;
					LastSection = Viewport->Actor->SongSection;
					debugf( TEXT("audio: music switch - Actor->Song=%s section=%i  Level->Song=%s"),
						Viewport->Actor->Song ? Viewport->Actor->Song->GetName() : TEXT("<none>"),
						(INT)Viewport->Actor->SongSection,
						(Viewport->Actor->Level && Viewport->Actor->Level->Song)
							? Viewport->Actor->Level->Song->GetName() : TEXT("<none>") );
				}
			}
			FScopedLock Lock( MusicMutex );
			MusicFade = 1.f;
